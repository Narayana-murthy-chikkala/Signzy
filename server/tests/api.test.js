...
describe('POST /routing-configs', () => {
  ...
  it('applies the described order as vendor priority when no percentages are present', async () => {
    await request(app).post('/vendors').send({ ...baseVendor, name: 'Vendor A', priority: 1 });
    await request(app).post('/vendors').send({ ...baseVendor, name: 'Vendor B', priority: 1 });
    await request(app).post('/vendors').send({ ...baseVendor, name: 'Vendor C', priority: 1 });

    const generated = await request(app).post('/ai-rule-generator').send({
      text: 'Use Vendor A as the primary vendor. If it times out or returns an error, automatically retry with Vendor B, then Vendor C.',
    });
    expect(generated.body.data.weights).toBeNull();
    expect(generated.body.data.vendorOrder).toEqual(['Vendor A', 'Vendor B', 'Vendor C']);

    const res = await request(app).post('/routing-configs').send(generated.body.data);

    expect(res.status).toBe(201);
    expect(res.body.data.appliedAs).toBe('priority');
    expect(res.body.data.appliedToVendors).toEqual(['Vendor A', 'Vendor B', 'Vendor C']);

    const vendors = await request(app).get('/vendors');
    const byName = Object.fromEntries(vendors.body.data.map((v) => [v.name, v.priority]));
    expect(byName['Vendor A']).toBeGreaterThan(byName['Vendor B']);
    expect(byName['Vendor B']).toBeGreaterThan(byName['Vendor C']);
  });
});
