...
const getVendorMetrics = async (req, res) => {
  const vendors = await Vendor.find().select('name healthStatus isActive currentLatency metrics');
  ...
  const summary = vendors.reduce(
    ...
    { totalVendors: 0, healthyVendors: 0, totalRequests: 0, totalSuccessful: 0, latencySum: 0 }
  );
